...
<section class="search_hero flex vc hc" style="background-image:url(<?php echo $background_image['url']; ?>);">
    <div class="container container-xsm flex row hc vc">
        <?php if($page_title):
                ...
                echo '<h1 class="archive_title"style="font-size: 50px;font-weight: 400;">EVENTS</h1>';
            endif;
		?>
...
<script>
	...
	jQuery(document).ready(function($){
        ...
        function resetFilters(){
            $('input[type=text], input[type=date], input').val('');
        ...
        function postSearch(cat, catName) {
            var postType = '';
            var selected = false;
            if($(".post-type:checked").val()) {
                postType = $(".post-type:checked").val();
                selected = true;
            } else {
            ...
            var datasearch = {
                    ...
                    event_location : $('#city option:selected').val(),
                    post_type : '<?php echo get_post_type(); ?>',
                    postsnum : 12
                };
                var cat_name = '';
