<?php
/**
 * The template for display a dress filter in the sidebar
 */
?>
<div class="wrapper">
    <?php 
    ...
    <?php 
    $designers = get_terms(array('taxonomy' => array('designers'), 'hide_empty' => false,));
    $i = 1;
    ?>
    <h3>Designers</h3>
    <ul class="filter-list designers">
        <?php foreach($designers as $cat) : ?>
            <li class="filter-category" data-id="<?php echo $cat->term_id; ?>"><input type="checkbox" data-twd-tax="designers" name="designers" value="<?php echo $cat->term_id; ?>" id="designer-<?php echo $i; ?>" class="br"><label for="designer-<?php echo $i; ?>"><?php echo $cat->name; ?></label></li>
        <?php
        $i++;
        endforeach;
        ?>
    ...
    <?php 
    $types = get_terms(array('taxonomy' => array('type'), 'hide_empty' => false,));
    $i = 1;
    ?>
    <h3>Dress Type</h3>
    <ul class="filter-list type">
        <?php foreach($types as $cat) : ?>
            <li class="filter-category" data-id="<?php echo $cat->term_id; ?>"><input type="checkbox" data-twd-tax="type" name="type" value="<?php echo $cat->term_id; ?>" id="type-<?php echo $i; ?>" class="br"><label for="type-<?php echo $i; ?>"><?php echo $cat->name; ?></label></li>
        <?php
        $i++;
        endforeach;
        ?>
    ...
    <?php dynamic_sidebar( 'sidebar-dress-listings' ); ?>
</div>